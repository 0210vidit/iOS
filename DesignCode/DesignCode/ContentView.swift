...
struct ContentView: View {
    ...
    @State var viewState = CGSize.zero //has width and height which we are setting to zero.
    ...
    var body: some View {
        ZStack {
            ...
            BackCardView()
                ...
                .scaleEffect(0.9).rotationEffect(.degrees(show ? 0 : 10))
            ...
            BottomCardView().blur(radius: show ? 20:0)
...
struct BottomCardView: View {
    var body: some View {
        VStack(spacing:20) {
            Rectangle()
                .frame(width:40,height: 5).cornerRadius(3).opacity(0.1)
            Text("This certificate is proof that Vidit Shah has achieved the UI Design course with approval from a Design+Code instructor")
